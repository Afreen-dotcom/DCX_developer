...
const groupedByState = developers.reduce((acc, dev) => {if (!acc[dev.state]) 
  acc[dev.state] = []; acc[dev.state].push(dev);return acc;}, {});
 
const sortedStates = Object.keys(groupedByState).sort((a, b) => a.localeCompare(b));
 

return <div className='card border-primary container mb-4' style={{
  maxWidth: "100%",
  width: "800px", 
...
>
{sortedStates.length>0? sortedStates.map((state) => (
  ...
  </div>)):<h4 className='text-center'>Developers Not Available</h4>
